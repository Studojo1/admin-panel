  ...
  async function toggleStats(item: SavedUTM) {
    ...
    try {
      const [visitorsRes, scrollRes, clicksRes] = await Promise.all([
        ...
        phQuery({
          kind: "HogQLQuery",
          query: `SELECT avg(toFloatOrDefault(properties.depth, 0.0)) FROM events WHERE properties.utm_campaign = '${safe}' AND event = 'scroll_depth' AND timestamp > now() - INTERVAL 90 DAY`,
        }),
        phQuery({
          kind: "HogQLQuery",
