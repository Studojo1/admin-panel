...
const KEY_STORAGE = "cc_admin_key";
...
export default function CareerCoachAdmin(_: Route.ComponentProps) {
  ...
  const [ccKey, setCcKey] = useState<string>(() => {
  ...
  // Auto-authenticate if key already stored
  useEffect(() => {
    if (ccKey) verifyKey(ccKey);
  }, []);

  async function verifyKey(key: string) {
    try {
      const res = await fetch(`${CC_API}/admin/overview`, {
  ...
  const ccHeaders = useCallback(
    () => ({ "x-admin-key": ccKey, "Content-Type": "application/json" }),
    [ccKey],
  );
  ...
  async function loadAll(key?: string) {
    ...
    setLoading(true);
    try {
      const headers = { "x-admin-key": k, "Content-Type": "application/json" };
    ...
    return (
      <div className="min-h-screen bg-neutral-50">
        ...
        <div className="flex min-h-[calc(100vh-80px)] items-center justify-center">
          <div className="w-80 rounded-3xl border-2 border-neutral-900 bg-white p-10 shadow-[6px_6px_0px_0px_rgba(25,26,35,1)]">
            <div className="mb-1 font-['Clash_Display'] text-xl font-black text-neutral-900">
              Career Coach
            </div>
            <div className="mb-2 inline-block rounded-full bg-violet-500 px-3 py-0.5 text-xs font-bold text-white">
              ADMIN
            </div>
            <p className="mb-6 text-sm text-neutral-500">Enter the CC admin key to access analytics.</p>
            <input
              type="password"
              className={`mb-3 w-full rounded-xl border-2 p-3 font-['Satoshi'] text-sm outline-none focus:border-violet-500 ${keyError ? "border-red-400" : "border-neutral-200"}`}
              placeholder="CC admin key…"
              value={keyInput}
              onChange={(e) => setKeyInput(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleKeySubmit()}
            />
            {keyError && (
              <p className="mb-3 text-xs text-red-500">Incorrect key — try again.</p>
            )}
            <button
              onClick={handleKeySubmit}
              className="w-full rounded-full border-2 border-neutral-900 bg-gradient-to-r from-violet-500 to-pink-500 py-3 font-['Satoshi'] text-sm font-bold text-white shadow-[3px_3px_0px_0px_rgba(25,26,35,1)] transition-all hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_0px_rgba(25,26,35,1)]"
            >
              Access Dashboard
            </button>
          </div>
